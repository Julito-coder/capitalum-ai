...
const NewSimulation = () => {
  ...
  const loadExistingProject = async (projectId: string) => {
    try {
      ...
      const defaults = getDefaultAdvancedState(project.type as 'LOCATIF' | 'RP');
      ...
      setAdvState({
        ...
        project: {
          ...
          title: project.title || '',
          city: project.city || '',
          postalCode: project.postal_code || undefined,
          surfaceM2: project.surface_m2 || 50,
          rooms: project.rooms || 2,
          propertyType: (project.property_type as 'apartment' | 'house' | 'building' | 'commercial') || 'apartment',
          condition: project.is_new ? 'new' : 'good',
        ...
        financing: {
          ...
          insuranceMode: financing?.insurance_mode === 'fixed' ? 'monthly' : 'initial_percent',
        ...
        rental: {
          ...defaults.rental,
          locationType: (project.strategy as 'nu' | 'meuble' | 'coloc' | 'saisonnier') || 'meuble',
          ...
          seasonalEnabled: rental?.is_seasonal || false,
          occupancyRate: rental?.seasonal_occupancy_rate || 70,
          nightlyRate: rental?.seasonal_avg_night || 80,
          platformFeesPct: rental?.seasonal_platform_fees || 15,
          cleaningFeePerStay: rental?.seasonal_cleaning_fees || 30,
        },
        ownerOccupier: {
          ...defaults.ownerOccupier,
        ...
        operatingCosts: {
          ...
          insurancePNO: operating_costs?.insurance_annual || 150,
          ...
          inflationRate: operating_costs?.costs_growth_rate || 2,
        },
        taxConfig: {
          ...defaults.taxConfig,
          taxMode: tax_config?.tax_mode === 'advanced' ? 'regime' : (tax_config?.tax_mode as 'simple' | 'regime' | 'override') || 'simple',
          ...
          amortizationComponents: ((tax_config?.amortization_components as unknown[]) || []).map((c: any) => ({
            name: c.name || '',
            valuePct: c.value_pct ?? c.valuePct ?? 0,
            durationYears: c.duration_years ?? c.durationYears ?? 0,
          })),
        ...
        stressSettings: defaults.stressSettings,
      });
