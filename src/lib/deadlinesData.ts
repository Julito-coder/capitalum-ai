...
export const FISCAL_DEADLINES: FiscalDeadline[] = [
  ...
  // ── Échéances étudiantes ──────────────────────────────────
  {
    key: 'bourse-crous-dse-2026',
    title: 'Bourse CROUS — Dépôt du DSE',
    shortDescription: 'Date limite pour déposer ton Dossier Social Étudiant et demander ta bourse.',
    explanation: `Le DSE (Dossier Social Étudiant) est la demande unique pour obtenir une bourse sur critères sociaux et/ou un logement CROUS. Il doit être constitué chaque année entre mars et mai. Sans dépôt, aucune bourse ni aide au logement CROUS ne sera versée à la rentrée.`,
    consequences: `Sans DSE déposé avant la date limite, tu ne pourras pas prétendre à la bourse CROUS pour l'année universitaire suivante. Perte potentielle de plusieurs milliers d'euros d'aide.`,
    category: 'administratif' as const,
    date: new Date(2026, 4, 15),
    impactScore: 5,
    estimatedImpact: 5000,
    relevanceCondition: (p) => p.isStudent,
    computePersonalImpact: () => ({
      estimatedGain: 5000,
      riskIfNoAction: 5000,
      patrimonialEffect10y: 15000,
      explanation: `La bourse CROUS peut atteindre ~500 €/mois sur 10 mois, soit ~5 000 €/an. Ne pas déposer le DSE = renoncer à cette aide.`,
    }),
    externalUrl: 'https://www.messervices.etudiant.gouv.fr/envole/',
    externalUrlLabel: 'Déposer mon DSE',
    actions: [
      { id: 'external-dse', label: 'Déposer mon DSE', icon: 'ExternalLink', type: 'external' as const, target: 'https://www.messervices.etudiant.gouv.fr/envole/' },
      { id: 'profile', label: 'Mettre à jour mon profil', icon: 'User', type: 'navigate' as const, target: '/profil-fiscal' },
    ],
    tags: ['étudiant', 'bourse', 'crous', 'dse'],
  },
  {
    key: 'bourse-crous-versement-2026',
    title: 'Bourse CROUS — 1er versement rentrée',
    shortDescription: 'Premier versement de la bourse CROUS pour la rentrée universitaire.',
    explanation: `Si ton DSE a été validé, le premier versement de la bourse CROUS est effectué début septembre, avant la rentrée. Vérifie que ton RIB est à jour sur ton espace CROUS pour éviter tout retard.`,
    consequences: `Un RIB manquant ou erroné retarde le versement de plusieurs semaines. Assure-toi que tout est en ordre.`,
    category: 'administratif' as const,
    ...
    computePersonalImpact: () => ({
      ...
      riskIfNoAction: 500,
      patrimonialEffect10y: 0,
      explanation: `Le premier versement mensuel de la bourse (~500 €) arrive début septembre si tout est en ordre.`,
    }),
    externalUrl: 'https://www.messervices.etudiant.gouv.fr/envole/',
    externalUrlLabel: 'Vérifier mon dossier',
    actions: [
      { id: 'external-crous', label: 'Mon espace CROUS', icon: 'ExternalLink', type: 'external' as const, target: 'https://www.messervices.etudiant.gouv.fr/envole/' },
    ],
    tags: ['étudiant', 'bourse', 'crous', 'versement'],
  },
  {
    key: 'apl-etudiant-caf-2026',
    title: 'APL étudiant — Demande CAF',
    shortDescription: 'Fais ta demande d\'APL dès ton emménagement pour réduire ton loyer.',
    explanation: `L'Aide Personnalisée au Logement (APL) peut couvrir une partie de ton loyer étudiant. La demande se fait en ligne sur caf.fr dès que tu as signé ton bail. Plus tu la fais tôt, plus vite tu reçois l'aide (attention : le 1er mois n'est pas versé).`,
    consequences: `Chaque mois sans demande = un mois d'aide perdu. L'APL n'est pas rétroactive. Perte moyenne : ~200 €/mois.`,
    category: 'administratif' as const,
    ...
    computePersonalImpact: () => ({
      ...
      riskIfNoAction: 2400,
      patrimonialEffect10y: 0,
      explanation: `L'APL étudiant représente ~200 €/mois, soit ~2 400 €/an d'aide au logement. Ne pas la demander = la perdre.`,
    }),
    externalUrl: 'https://www.caf.fr/allocataires/mes-services-en-ligne/faire-une-demande-de-prestation',
    externalUrlLabel: 'Demander mon APL',
    actions: [
      { id: 'external-caf', label: 'Demander sur caf.fr', icon: 'ExternalLink', type: 'external' as const, target: 'https://www.caf.fr/allocataires/mes-services-en-ligne/faire-une-demande-de-prestation' },
      { id: 'profile', label: 'Mettre à jour mon profil', icon: 'User', type: 'navigate' as const, target: '/profil-fiscal' },
    ],
    tags: ['étudiant', 'apl', 'caf', 'logement'],
  },
  {
    key: 'cvec-paiement-2026',
    title: 'CVEC — Paiement contribution étudiante',
    shortDescription: 'Paye la Contribution Vie Étudiante et de Campus avant ton inscription.',
    explanation: `La CVEC (103 €) est obligatoire pour s'inscrire dans un établissement d'enseignement supérieur. Les boursiers en sont exonérés mais doivent quand même obtenir l'attestation. Le paiement ou l'attestation d'exonération est requis AVANT l'inscription administrative.`,
    consequences: `Sans attestation CVEC, impossible de finaliser ton inscription universitaire. Risque de retard d'inscription.`,
    category: 'administratif' as const,
    date: new Date(2026, 8, 1),
    impactScore: 3,
    estimatedImpact: -103,
    ...
    computePersonalImpact: () => ({
      ...
      riskIfNoAction: 103,
      patrimonialEffect10y: 0,
      explanation: `La CVEC coûte 103 € (gratuite pour les boursiers). Sans attestation, ton inscription sera bloquée.`,
    }),
    externalUrl: 'https://cvec.etudiant.gouv.fr/',
    externalUrlLabel: 'Payer la CVEC',
    actions: [
      { id: 'external-cvec', label: 'Payer / obtenir attestation', icon: 'ExternalLink', type: 'external' as const, target: 'https://cvec.etudiant.gouv.fr/' },
    ],
    tags: ['étudiant', 'cvec', 'inscription'],
  },
  {
    key: 'parcoursup-voeux-2026',
    title: 'Parcoursup — Confirmation des vœux',
    shortDescription: 'Date limite pour confirmer tes vœux sur Parcoursup.',
    explanation: `La confirmation des vœux sur Parcoursup est une étape cruciale. Chaque vœu non confirmé avant la date limite est automatiquement supprimé. Vérifie que tous tes dossiers sont complets (lettres de motivation, bulletins, pièces justificatives).`,
    consequences: `Tout vœu non confirmé est définitivement supprimé. Aucun recours possible après la date limite.`,
    category: 'administratif' as const,
    date: new Date(2026, 3, 2),
    impactScore: 5,
    estimatedImpact: 0,
    relevanceCondition: (p) => p.isStudent || p.childrenCount > 0,
    computePersonalImpact: (p) => ({
      estimatedGain: 0,
      riskIfNoAction: 0,
      patrimonialEffect10y: 0,
      explanation: p.isStudent
        ? `Confirme chacun de tes vœux avant le 2 avril. Un vœu non confirmé est supprimé sans recours.`
        : `Si ton enfant est en terminale, assure-toi que ses vœux Parcoursup sont tous confirmés avant le 2 avril.`,
    }),
    externalUrl: 'https://www.parcoursup.fr/',
    externalUrlLabel: 'Accéder à Parcoursup',
    actions: [
      { id: 'external-parcoursup', label: 'Ouvrir Parcoursup', icon: 'ExternalLink', type: 'external' as const, target: 'https://www.parcoursup.fr/' },
    ],
    tags: ['étudiant', 'parcoursup', 'orientation', 'enfants'],
  },
];
